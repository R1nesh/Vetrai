...
export function LiveDataTab() {
    ...
    const interval = setInterval(() => {
      setLiveStats((prev) => ({
        avgSpeed: Math.max(
          ...
          Math.min(
            45,
            prev.avgSpeed + (Math.random() - 0.5) * 3,
          ),
        ),
        ...
        congestionLevel: Math.max(
          ...
          Math.min(
            100,
            prev.congestionLevel + (Math.random() - 0.5) * 5,
          ),
        ),
        camerasOnline: Math.max(
          140,
          Math.min(
            160,
            prev.camerasOnline +
  ...
  return (
    <div className="space-y-6">
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-white text-3xl mt-1">
                    {liveStats.avgSpeed.toFixed(1)}
                  </h3>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h3
                    className={`text-3xl mt-1 ${getCongestionColor(liveStats.congestionLevel)}`}
                  >
                ...
                <TrendingUp
                  className={`w-8 h-8 ${getCongestionColor(liveStats.congestionLevel)}`}
                />
              </div>
            </CardContent>
          </Card>
      ...
      {/* Live Traffic Map */}
      <Card className="relative z-0 bg-gray-900 border-gray-800">
        <CardHeader>
          <div className="flex items-center justify-between">
          ...
          <CardDescription className="text-gray-400">
            Live traffic flow with real-time updates every 30
            seconds
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="bg-gray-900 border-gray-800">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-white">
              Traffic Projection - Next 72 Hours
            </CardTitle>
          </div>
          <CardDescription className="text-gray-400">
            AI-powered prediction of traffic patterns based on
